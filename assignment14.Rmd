...
df2 <- find_thread_urls(keywords="cute kittens", subreddit="cats", sort_by="new", period="month")
write_csv(df2, "reddit_cats.csv")
df = read_csv("reddit_cats.csv")
df2 %>% 
  unnest_tokens(input = title, output = word) %>% 
  anti_join(get_stopwords()) %>% 
  count(word, sort = TRUE) %>% 
...
df %>%
    ...
    group_by(threads) %>% 
    mutate(n = n/sum(n)) %>% 
    ggplot(aes(threads, n, fill=sentiment))+geom_col(position = 'dodge')+
    labs(y='Relative Frequency', x ='')
