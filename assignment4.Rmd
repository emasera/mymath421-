...
```{r}
df3 = read_csv("C:/Users/student/Desktop/2023-2024/Fall Semester/Statistical Analysis with R/mymath421-/adult_census.csv")
df3 %>% select(-race)
df3 %>% filter(education == "HS-grad")
df3 %>% mutate(total_gain = capital.gain - capital.loss)
df3 %>% summarise(mean_education = mean(education.num))
df3 %>% arrange(education.num)
df3 %>% count(education.num)
df3 %>% count(education.num) %>% arrange(-education.num)
df3 %>% filter(workclass == "Private") %>% count(education.num) %>% arrange(-education.num)
df3 %>% group_by(education.num) %>% summarise(min_hours = min(hours.per.week))
df3 %>% filter(workclass == "Private") %>% group_by(education.num) %>% 
